...
struct StreamingFullQRD {
    ...
    void operator()() const {
        ...
        */
        ...
        // Compute QRDs as long as matrices are given as inputs (or max iteration reached)
        for (int iteration = 0; iteration < iteration_count; iteration ++) {
            ...
            // Output R matrix to pipe ------------------------------------------------------
            constexpr int fRMatrixSize = columns * columns;
            constexpr int RExtraIteration = (columns % pipe_size) != 0 ? 1 : 0;
            constexpr int RLoopIterPerColumn = columns / pipe_size + RExtraIteration;
            // Number of pipe reads of pipe_size to read all the matrices
            constexpr int RLoopIter = RLoopIterPerColumn * columns;
            // Size in bits of the loop iterator over kLoopIter iterations
            constexpr int RLoopIterBitSize =
                                        fpga_tools::BitsForMaxValue<RLoopIter + 1>();
            // first convert r_result into full-size r matrix r_full (padding with zeros and col_maj)
            TT r_full[fRMatrixSize];
            [[intel::initiation_interval(1)]]  // NO-FORMAT: Attribute
            for (int r_idx = 0; r_idx < fRMatrixSize; r_idx++) 
            {
                int cur_col = r_idx / columns;
                int cur_row = r_idx % columns;

                // if in lower triangle
                if (cur_col < cur_row)
                {
                    r_full[r_idx] = 0.0;
                }
                ...
                {
                    // some math to map r_idx back to r_result idx
                    r_full[r_idx] = r_result[cur_row * columns + cur_col - (int)(cur_row * ((cur_row + 1) * 0.5))];
                }
            }

            // write r_full into pipe
            int load_index = 0;
            [[intel::initiation_interval(1)]]  // NO-FORMAT: Attribute
            for (ac_int<RLoopIterBitSize, false> li = 0; li < RLoopIter; li++) 
            {
                bool last_burst_of_col;
                if constexpr (RExtraIteration)
                {
                    // Check if we are reading the last DDR burst of the current column
                    last_burst_of_col = (li % RLoopIterPerColumn)
                                        == RLoopIterPerColumn - 1;
                }

                fpga_tools::NTuple<TT, pipe_size> ddr_read;
                // Perform the DDR burst read of pipe_size elements
                fpga_tools::UnrolledLoop<pipe_size>([&](auto k) 
                {
                    if constexpr (RExtraIteration)
                    {
                        ...
                        bool out_of_bounds = last_burst_of_col &&
                                ((k % pipe_size) > ((columns - 1) % pipe_size));

                        // Only perform the DDR reads that are relevant (and don't access a
                        // memory address that may be beyond the matrix last address)
                        if (!out_of_bounds) 
                        {
                            ddr_read.template get<k>() = r_full[load_index + k];
                        }
                    }
                    else
                    {
                        ddr_read.template get<k>() = r_full[(int)(li)*pipe_size + k];
                ...
                if constexpr (RExtraIteration)
                {
                    ...
                    load_index += last_burst_of_col ? columns % pipe_size :
                                                    pipe_size;
                }
                ROut::write(ddr_read);
            }
